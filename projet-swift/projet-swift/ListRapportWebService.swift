...
class ListRapportWebService {
    ...
    static func getListRapportImm(token: String) async -> [String: AnyObject]  {
        
        var dic:[String:AnyObject] = [:]
        dic["error"] = 1 as AnyObject
        dic["message"] = "Impossible de récupérer les rapports" as AnyObject
        
        var request = URLRequest(url: URL(string: "http://localhost:3000/api/user/rapport/liste/immeuble/")!)
        let parameters = "token=" + token
